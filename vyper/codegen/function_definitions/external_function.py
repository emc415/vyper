...
def _register_function_args(context: Context, func_t: ContractFunctionT) -> List[IRnode]:
    ...
    base_args_t = TupleT(tuple(arg.typ for arg in func_t.base_args))

    # tuple with the abi_encoded args
    #TODO: what does this do? emc 2.7.23
    ...
    for i, arg in enumerate(func_t.base_args):
...
#TODO check this function that you got your default and base args correct in class ContractFunctionT
def _generate_kwarg_handlers(context: Context, func_t: ContractFunctionT) -> List[Any]:
    ...
    def handler_for(calldata_kwargs, default_kwargs):
        ...
        abi_func = func_t.abi_signature_for_kwargs(calldata_kwargs)
        method_id = _annotated_method_id(abi_func)
        ...
        ret.append(["goto", func_t.external_function_base_entry_label])
    ...
    keyword_args = func_t.default_args
...
def generate_ir_for_external_function(code, func_t, context, skip_nonpayable_check):
    ...
    if func_t.mutability != StateMutability.PAYABLE and not skip_nonpayable_check:
    ...
    body = ["label", func_t.external_function_base_entry_label, ["var_list"], body]
    ...
    exit = ["label", func_t.exit_sequence_label, exit_sequence_args, exit_sequence]
